...
class Grid {
  ...
  // GRID METHOD: Draws the initial grid with obstacles and rovers in the canvas
    ...
    // Sets the origin for the rovers
		...
		for (var enemyOri in rover.enemy) {
			rover.enemy[enemyOri].setOrigin(this);
		}
            ...
            this.cnv.height / this.size);
					...
					for (var enemyCoor in rover.enemy) {
						...
						if (rover.enemy[enemyCoor].position[0] === i &&
							...
							rover.enemy[enemyCoor].drawRotatedImage(this);
							continue;
						}
					}
				}
    ...
    // Puts the enemies in automated move
		for (var enemyAuto in rover.enemy) {
			rover.enemy[enemyAuto].autoMove(this);
		}
	}
}
...
class Rover {
  ...
  // ROVER METHOD: Sets the starting position and direction of the rover
	setOrigin(grid) {
		...
		if (grid.isBlocked(this.position[0], this.position[1]) ||
			this.isSame(grid, this.position[0], this.position[1]) ||
      this.isOther(grid, this.position[0], this.position[1])) {
			this.setOrigin(grid);
		}
  ...
  // ROVER METHOD: Checks if a given position is blocked by other rover (one of them is player1)
	...
	isSame(grid, firstCoord, secondCoord) {
		for (var i in Rover.allInstances) {
			if ((Rover.allInstances[i] !== this) &&
				(this.automated === Rover.allInstances[i].automated) &&
				(firstCoord === Rover.allInstances[i].position[0]) &&
  ...
  // ROVER METHOD: Runs if grid.isBlocked is true in goForward or goBack
	runIfBlocked(grid) {
		if (!this.automated) {
  ...
  // ROVER METHOD: Runs if isOther is true in goForward or goBack
	runIfOther(grid) {
		...
		console.log("Player was destroyed by Alien");
  ...
  // ROVER METHOD: Moves forward (towards the current direction)
    ...
    // In case the nextPosition is blocked by an obstacle, the rover explodes and loses the game
		if (grid.isBlocked(this.nextPosition[0], this.nextPosition[1])) {
			...
			return;
		} else if (this.isSame(grid, this.nextPosition[0], this.nextPosition[1])) {

			// In case the rovers to collide are both automated, exit the method
			return;
		} else if (this.isOther(grid, this.nextPosition[0], this.nextPosition[1])) {

			// In case the player is colliding with an enemy
...
"darkblue", player1, 0, "#value2", true);
